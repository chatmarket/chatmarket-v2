...
Deno.serve(async (req) => {
  try {
    ...
    // ★ AWS クォータ回避：固定チャンネルを再利用
    // （毎回チャンネル作成ではなく、既存チャンネルを使用し stream-key は上書き）
    ...
    console.log(`[createLiveStream] ✅ Using fixed channel (quota-safe mode):`, {
      channelArn: FIXED_CHANNEL_ARN,
      playbackUrl: FIXED_PLAYBACK_URL.substring(0, 80) + '...',
    });
