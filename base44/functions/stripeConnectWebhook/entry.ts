 ...
 * Stripe Connect アカウント関連イベントを処理する。
 * - account.updated: オンボーディング完了検知 → stripe_connect_status を 'active' に更新
 * - checkout.session.completed (v2): Connect経由の寄付完了処理
 */
...
Deno.serve(async (req) => {
  try {
    ...
    // ── account.updated: Connectオンボーディング完了 ──
    if (event.type === 'account.updated') {
      const account = event.data.object;
      const isReady = account.charges_enabled && account.payouts_enabled && account.details_submitted;

      if (isReady && account.metadata?.project_id) {
        const projectId = account.metadata.project_id;
        await base44.asServiceRole.entities.CrowdfundingProject.update(projectId, {
          stripe_connect_status: 'active',
          stripe_connect_account_id: account.id,
        });
        console.log(`[ConnectWebhook] account ${account.id} → active (project: ${projectId})`);
      }
      return Response.json({ ok: true });
    }

    // ── checkout.session.completed (v2寄付) ──
    if (event.type === 'checkout.session.completed') {
      const session = event.data.object;
      const meta = session.metadata || {};

      if (meta.type !== 'crowdfunding_donation_v2') {
      ...
      await base44.asServiceRole.entities.CreatorEarning.create({
        ...
        coin_amount:         0,
        progressive_rate:    storedRate,
        description:         `[Connect] クラウドファンディング寄付 ¥${amount} → NPO振込¥${payoutYen} (率${Math.round(storedRate * 100)}%)`,
      });
      ...
      if (channel) {
        ...
        await base44.asServiceRole.entities.Channel.update(channel.id, {
          ...
          monthly_revenue_coins: Math.floor(monthlyYen / 1.1),
          rate_applied_month: now.toISOString().slice(0, 7),
        });
        console.log(`[ConnectWebhook] ${ownerEmail} 月間¥${monthlyYen} → ${Math.round(newRate * 100)}% 適用`);
      }
    ...
    console.error('[stripeConnectWebhook]', err.message);
